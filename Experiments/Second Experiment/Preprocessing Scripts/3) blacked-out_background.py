...
output_dir = os.path.join(project_root, r"_data/datasets/train_blacked-out_background")
...
def enhance_dog_image(image_path):
    ...
    dog_mask = np.zeros(image.size[::-1], dtype=np.uint8)
    ...
    background_only = np.zeros_like(image_cv)
